    ...
    int longestConsecutive(vector<int>& nums) {
        ...
        for (int num: nums) { // num = 3 , num = 5
            if (pastSet.find(num - 1) != pastSet.end()) { // found // found num = 2, num = 5 is in set
                int length = 1;
                while (pastSet.find(num + length) != pastSet.end()){ // continue loop bit start from the num which start the sequece . My error is start from the current num which not the start of sequence so the length max is not the length of the sequence
                    length++;
                }
